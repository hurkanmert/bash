...
do
	...
	echo "printf '%-20s %-20s %-20s %-20s %-20s %-20s %-20s$n' Obsid Down_start_date Down_start_time Down_time_second Down_file_size Number_of_file Download_speed_in_per_second>> /next/lmxbs/"$new"/"$new"_info.txt" >> /next/lmxbs/$new/$new"_download_script.sh"
	...
	do
		...
		((ii=i))
  		a=$(sed -n "$i"p "$f" | cut -d '-' -f 1)
  		b=$(sed -n "$i"p "$f")
  		c=$(sed -n "$i"p "$f" | cut -b 1)
  		d=$(sed -n "$i"p "$f" | cut -b 2)
 	 	g=$(sed -n "$i"p "$f" | cut -b 1-2)
 		((i=i+1))
  		...
  		if [ "$g" -ge 91 ]; then
			...
			url1="https://heasarc.gsfc.nasa.gov/FTP/xte/data/archive/AO1"$num"//P"$a"/"$b"/."

			if wget $url1 >/dev/null 2>&1 ; then
				printf " %-1s/%-4s %-2s: %-10s  %-10s$n" $data $ii Obsid $b Exists.
				rm index*

				((number_exist=i-1))

				echo "echo "$b" Downloading..." >> /next/lmxbs/$new/$new"_download_script.sh"
  				echo "obsid="$b"" >> /next/lmxbs/$new/$new"_download_script.sh"
				echo 'date='$o'(date "+%Y-%m-%d %H:%M:%S")' >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "start="$o"(date +%s)" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "wget -q -nH --no-check-certificate --cut-dirs=5 -r -l0 -c -N -np -R 'index*' -erobots=off --retr-symlinks https://heasarc.gsfc.nasa.gov/FTP/xte/data/archive/AO1"$num"//P"$a"/"$b"/." >> /next/lmxbs/$new/$new"_download_script.sh"
   				echo "end="$o"(date +%s)" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "runtime="$o"((end-start))" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "size="$o"(du -ch /next/lmxbs/$new/P$a/$b/ | tail -1 | cut -f1)" >> /next/lmxbs/$new/$new"_download_script.sh"
					echo "download_speed=0" >> /next/lmxbs/$new/$new"_download_script.sh"
					echo "((download_speed=runtime/size))" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "nof="$o"(ls /next/lmxbs/$new/P$a/$b/ | wc -l)" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "printf '%-20s %-20s %-20s %-20s %-20s %-20s %-20s$n' "$o"obsid "$o"date "$o"runtime "$o"size "$o"nof "$o"download_speed >> /next/lmxbs/"$new"/"$new"_info.txt" >> /next/lmxbs/$new/$new"_download_script.sh"
				echo "" >> /next/lmxbs/$new/$new"_download_script.sh"

			else
				printf " %-1s/%-4s %-2s: %-10s  %-10s$n" $data $ii Obsid $b "Does not exists."
			...
			url2="https://heasarc.gsfc.nasa.gov/FTP/xte/data/archive/AO9//P"$a"/"$b"/."
			...
			if wget $url2 >/dev/null 2>&1 ; then
				...
				echo 'date='$o'(date "+%Y-%m-%d %H:%M:%S")' >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "start="$o"(date +%s)" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "wget -q -nH --no-check-certificate --cut-dirs=5 -r -l0 -c -N -np -R 'index*' -erobots=off --retr-symlinks https://heasarc.gsfc.nasa.gov/FTP/xte/data/archive/AO9//P"$a"/"$b"/." >> /next/lmxbs/$new/$new"_download_script.sh"
   				echo "end="$o"(date +%s)" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "runtime="$o"((end-start))" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "size="$o"(du -ch /next/lmxbs/$new/P$a/$b/ | tail -1 | cut -f1)" >> /next/lmxbs/$new/$new"_download_script.sh"
					echo "download_speed=0" >> /next/lmxbs/$new/$new"_download_script.sh"
    		...
    		url3="https://heasarc.gsfc.nasa.gov/FTP/xte/data/archive/AO"$c"//P"$a"/"$b"/."
			...
			if wget $url3 >/dev/null 2>&1 ; then
				...
				echo 'date='$o'(date "+%Y-%m-%d %H:%M:%S")' >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "start="$o"(date +%s)" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "wget -q -nH --no-check-certificate --cut-dirs=5 -r -l0 -c -N -np -R 'index*' -erobots=off --retr-symlinks https://heasarc.gsfc.nasa.gov/FTP/xte/data/archive/AO"$c"//P"$a"/"$b"/." >> /next/lmxbs/$new/$new"_download_script.sh"
   				echo "end="$o"(date +%s)" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "runtime="$o"((end-start))" >> /next/lmxbs/$new/$new"_download_script.sh"
    			echo "size="$o"(du -ch /next/lmxbs/$new/P$a/$b/ | tail -1 | cut -f1)" >> /next/lmxbs/$new/$new"_download_script.sh"
					echo "download_speed=0" >> /next/lmxbs/$new/$new"_download_script.sh"
